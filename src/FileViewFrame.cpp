...
FileViewFrame::FileViewFrame(const TGWindow* p, const TGFrame* main, UInt_t w, UInt_t h, SPSPlotMainFrame *parent, int type) {
	...
	TGLabel *nameLabel = new TGLabel(NameFrame, "File: ");
	...
	fMain->AddFrame(fViewer, fhints);
	fMain->AddFrame(NameFrame, fhints);
	fMain->AddFrame(ButtonFrame, fhints);

	/*Send signal to appropriate location*/
	if(type == SPSPlotMainFrame::M_SAVE_CONFIG) Connect("SendText(const char*)","SPSPlotMainFrame",parent,"WriteConfig(const char*)");
	else if(type == SPSPlotMainFrame::M_LOAD_CONFIG) Connect("SendText(const char*)","SPSPlotMainFrame",parent,"LoadConfig(const char*)");
	...
	fContents->SetFilter("*.inp"); //relevant extension
...
void FileViewFrame::DoOk() {
	...
	const char* filename = fNameField->GetText();
	if(strncmp(filename,"",1) == 0) { //check validity
...
void FileViewFrame::DoDoubleClick(TGLVEntry *entry, int id) {
	...
	if(entryname.EndsWith(".inp")) { //check if its a file
